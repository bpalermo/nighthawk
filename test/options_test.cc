...
TEST_F(OptionsImplTest, GrpcStreamDefaultsAndRoundTrip) {
  ...
  // 20 streams over 2 workers: the pending-request breaker is raised to hold 10 opening streams.
  EXPECT_EQ(10, options->maxPendingRequests());

  CommandLineOptionsPtr cmd = options->toCommandLineOptions();
  ASSERT_TRUE(cmd->has_grpc_stream());
  EXPECT_EQ(20, cmd->grpc_stream().streams().value());
...
TEST_F(OptionsImplTest, TunnelModeHInvalidProtocolCombination) {
  // not implemented in envoy.
  EXPECT_THROW_WITH_REGEX(
      TestUtility::createOptionsImpl(
          fmt::format("{} {} --protocol http3 --tunnel-protocol http1 --tunnel-uri http://foo/",
                      client_name_, good_test_uri_)),
      MalformedArgvException, "--protocol HTTP3 over --tunnel-protocol HTTP1 is not supported");
  ...
  EXPECT_THROW_WITH_REGEX(
      TestUtility::createOptionsImpl(fmt::format("{} {} --protocol http3 --tunnel-protocol http3 "
...
TEST_F(OptionsImplTest, TunnelModeMissingParams) {
  ...
  // test missing TLS context for H3 tunnel
  EXPECT_THROW_WITH_REGEX(
      TestUtility::createOptionsImpl(
          fmt::format("{} {} --protocol http2 --tunnel-protocol http3 --tunnel-uri http://foo/",
                      client_name_, good_test_uri_)),
      MalformedArgvException, "--tunnel-tls-context is required to use --tunnel-protocol http3");
}
